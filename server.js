...
var curRoom
    io.on('connection',function(socket){
      ...
      socket.on('logout',()=>{
          usersInChat.findOneAndRemove({"socketId": socket.id},function(err,deletedUser){
              console.log('deleted',deletedUser)
                if(err) {console.log(err)}
                else if(deletedUser){
                    socket.broadcast.to(deletedUser.room).emit('message',{user:'admin', text: `${deletedUser.name} has just left`,presence: 0})
                    usersInChat.find()
                    .then(users=>{
